...
function resolveCategory(type, scope) {
  ...
  if (scope) {
    ...
    if (MAINTENANCE_SCOPES.test(scope)) return '🔧 Maintenance';
    // chore with a deps/deps-dev scope → Dependencies
    if (type === 'chore' && scope.startsWith("deps")) return '__deps__';
  }
...
const logOutput = run(`git log ${range} --pretty=format:"%H%x09%s%x09%B%x00" --`);
